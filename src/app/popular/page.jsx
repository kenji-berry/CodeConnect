...
async function getInitialPopularProjectIds(limit = 100) {
  try {
    ...
    if (error) {
      console.error('Error fetching initial popular project IDs:', error);
      return [];
    }
    return (data || []).map(item => item.project_id).filter(id => id !== undefined);
  } catch (error) {
    console.error('Error in getInitialPopularProjectIds:', error);
    return [];
  }
}
...
function PopularProjectsContent() {
  ...
  const fetchPopularAndFilteredProjects = useCallback(async (page) => {
    setLoading(true);
    try {
      const popularProjectIds = await getInitialPopularProjectIds(100);

      if (!Array.isArray(popularProjectIds) || popularProjectIds.length === 0) {
        setProjects([]);
        setTotalPages(1);
        setLoading(false);
        return;
      ...
      if (rpcError) {
        console.error("Error fetching filtered popular project IDs:", rpcError);
        setProjects([]);
        setTotalPages(1);
        setLoading(false);
        return;
      ...
      console.error("Unexpected error fetching popular projects:", error);
      setProjects([]);
      setTotalPages(1);
    } finally {
      setLoading(false);
  ...
  return (
    <Suspense fallback={
        <div className="flex items-center justify-center min-h-screen w-full radial-background">
            ...
            <h1 className="inria-sans-bold text-xl text-off-white">Loading Popular Projects</h1>
            </div>
        </div>
    }>
