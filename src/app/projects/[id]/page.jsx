...
const ProjectDetails = () => {
  ...
  const getDifficultyLabel = (level) => {
    ...
    return difficultyLabels[level] || `Level ${level}`;
  };
      ...
      <div className="w-full max-w-[1200px] mx-auto">
        ...
        <section className="rounded-xl shadow-lg bg-[#232323] border border-[var(--muted-red)] p-8 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
            ...
            <div>
              <h3 className="font-semibold text-[--orange] mb-1">Difficulty Levels</h3>
              <div className="flex flex-wrap items-center gap-2">
                {Array.isArray(project.difficulty_level) && project.difficulty_level.length > 0 ? (
                  project.difficulty_level.map((level) => (
                    <div 
                      key={level} 
                      className="w-8 h-8 rounded-full bg-[color:--muted-red] text-white flex items-center justify-center font-medium"
                  ...
                  <span className="text-gray-400">No difficulty level specified</span>
                )}
              </div>
            </div>
